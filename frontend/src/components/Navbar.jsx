  ...
  return (
    <header className="w-full h-14 bg-gray-900 text-amber-50">
      <nav className="h-14 flex justify-between items-center px-4 relative">
        ...
        >
          <div className={`w-6 h-0.5 bg-amber-50 mb-2 transition-transform duration-500 ${isOpen ? "rotate-45 translate-y-1.5" : ""}`} />
          <div className={`w-6 h-0.5 bg-amber-50 mb-2 transition-opacity duration-500 ${isOpen ? "opacity-0" : "opacity-100"}`} />
          <div className={`w-6 h-0.5 bg-amber-50 transition-transform duration-500 ${isOpen ? "-rotate-45 -translate-y-3.5" : ""}`} />
        </button>
